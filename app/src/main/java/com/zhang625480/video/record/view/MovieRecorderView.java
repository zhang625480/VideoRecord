...
import com.zhang625480.video.record.application.VideoRecordApplication;
...
public class MovieRecorderView extends LinearLayout implements OnErrorListener {
    ...
    public void initCamera() {
        ...
        try {
            if (isFacingFront && checkCameraFacing(Camera.CameraInfo.CAMERA_FACING_FRONT)) {
                camera = Camera.open(Camera.CameraInfo.CAMERA_FACING_FRONT);//TODO 默认打开前置摄像头
            } else if (!isFacingFront && checkCameraFacing(Camera.CameraInfo.CAMERA_FACING_BACK)) {
            ...
            setCameraParams();
            camera.setDisplayOrientation(90);//设置 90度  否则输出后会反转
...
//        params.setPictureSize(480, 720);//拍照保存比率
    ...
    private void createRecordDir() {
        File sampleDir = new File(Environment.getExternalStorageDirectory() + File.separator + "newlife/video/");
        if (!sampleDir.exists()) {
            sampleDir.mkdirs();
        }
        try {
...
//        LogUtil.e(LOG_TAG,"手机支持的最大像素supportedPictureSizes===="+sizePicture);
        ...
        if (isFacingFront) {
            ...
            mediaRecorder.setOrientationHint(90);//输出旋转270度，保持竖屏录制
        }
